...
function SohoButton({ onClick, value, size = 'sm',block = false, style, disabled = false, ...prop }: Props) {
    const resolveSize = () => {
        if (size == "sm") return 14
        if (size == "lg") return 18
    }
    return (
            <div {...prop} className="upload-btn-wrapper" style={{ borderColor: disabled ? "#9f9f9f" : "#cf2c6b", color: disabled ? "#9f9f9f" : "#cf2c6b", display: 'table',width: block ? "100%": "unset", ...style }}>
